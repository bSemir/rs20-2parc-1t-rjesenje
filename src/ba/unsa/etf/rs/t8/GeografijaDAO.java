...
public class GeografijaDAO {
    ...
    private GeografijaDAO() {
        ...
        try {
            glavniGradUpit = conn.prepareStatement("SELECT grad.id, grad.naziv, grad.broj_stanovnika, grad.drzava, grad.datum_osnivanja FROM grad, drzava WHERE grad.drzava=drzava.id AND drzava.naziv=?");
        } catch (SQLException e) {
            regenerisiBazu();
            try {
                glavniGradUpit = conn.prepareStatement("SELECT grad.id, grad.naziv, grad.broj_stanovnika, grad.drzava, grad.datum_osnivanja FROM grad, drzava WHERE grad.drzava=drzava.id AND drzava.naziv=?");
        ...
        try {
            ...
            promijeniGradUpit = conn.prepareStatement("UPDATE grad SET naziv=?, broj_stanovnika=?, drzava=?, datum_osnivanja=? WHERE id=?");
    ...
    private Grad dajGradIzResultSeta(ResultSet rs, Drzava d) throws SQLException {
        ...
        return new Grad(rs.getInt(1), rs.getString(2), rs.getInt(3), d, date);
    }
    ...
    public void dodajGrad(Grad grad) {
        try {
            ...
            dodajGradUpit.setDate(5, Date.valueOf(grad.getDatumOsnivanja()));
    ...
    public void izmijeniGrad(Grad grad) {
        try {
            ...
            promijeniGradUpit.setDate(4, Date.valueOf(grad.getDatumOsnivanja()));
